...
f_old = Function_definition(V, 'Function', number_of_equations) # Defining list of variables for source term
f_old1 = Function_definition(V, 'Function', number_of_equations) # Defining list of variables for source term

muE = Function_definition(V, 'Function', number_of_equations) # Defining variable for stabilization
...
Gamma_old[1] = Flux(sign[1], u_old[1], D[1], mu[1], E) 
...
f_old[0] = alpha*mu[1]*E_m*u_old[1] # Ion source term definition
f_old[1] = alpha*mu[1]*E_m*u_old[1] # Electron source term definition

f_old1[0] = alpha*mu[1]*E_m*u_old1[1] # Ion source term definition
f_old1[1] = alpha*mu[1]*E_m*u_old1[1] # Electron source term definition
...
while i < number_of_species:
    ...
    if equation_type[i] == 'drift-diffusion-reaction':
        L_star = ( -D[i] *  div(df.grad(v[i]) ) - dot(muE[i], df.grad(v[i])) )

        F += weak_form_taylor_balance_equation(
    ...
    # F += weak_form_taylor_balance_equation(equation_type[i], dt, dt_old, dx, u[i], v[i], f[i], muE[i], Gamma[i], f_old=None, f_old1=None, r=r, D=D[i]) # Setting up variational formulation of electron energy balance equations
    i += 1
